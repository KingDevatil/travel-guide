...
export function itineraryGeoJson(stops: Stop[], legs: Leg[], date?: string): FeatureCollection<Point | LineString> {
  ...
  for (const leg of legs) { const from = visible.find((stop) => stop.id === leg.fromStopId); const to = visible.find((stop) => stop.id === leg.toStopId); if (from && to) features.push({ type: "Feature", properties: { id: leg.id, mode: leg.mode }, geometry: leg.routeGeoJson ?? { type: "LineString", coordinates: [[from.longitude, from.latitude], [to.longitude, to.latitude]] } }); }
