...
export function ItineraryTimeline({ trip, showMap = false }: { trip: Trip; showMap?: boolean }) {
  ...
  return <section className="data-timeline" aria-label="行程节点管理">
    ...
    {loading ? <p>正在加载行程…</p> : stops.length === 0 ? <div className="itinerary-empty"><MapPin aria-hidden="true" /><strong>还没有行程节点</strong><p>从搜索城市开始，坐标会自动匹配。</p><button onClick={() => setEditingStop(null)}>添加第一个节点</button></div> : dates.map((date) => {
      const dayStops = stops.filter((stop) => stop.date === date);
      return <div className="data-timeline__day" key={date}><h4>{date}</h4>{dayStops.length === 0 ? <p className="empty-day">暂无安排</p> : dayStops.map((stop, index) => <article className="data-timeline__stop" key={stop.id}><div><strong>{stop.title}</strong><span>{stop.address || [stop.city, stop.country].filter(Boolean).join("，") || "地图选点"}</span>{stop.address && <span>{[stop.city, stop.country].filter(Boolean).join("，")}</span>}{stop.startsAt && <span>{stop.startsAt} {stop.endsAt ? `— ${stop.endsAt}` : ""}</span>}</div><div><button onClick={() => setEditingStop(stop)}>编辑</button><button disabled={index === 0} onClick={() => void moveStop(stop, -1)}>上移</button><button disabled={index === dayStops.length - 1} onClick={() => void moveStop(stop, 1)}>下移</button><button onClick={() => setPendingDelete(stop)}>删除</button></div></article>)}</div>;
    })}
    ...
    {editingStop !== undefined && <StopEditor stop={editingStop ?? undefined} date={trip.startDate} tripStartDate={trip.startDate} tripEndDate={trip.endDate} existingStops={stops} onSave={(draft) => saveStop(draft, editingStop ?? undefined)} onClose={() => setEditingStop(undefined)} />}
