import type { Leg, PackingItem, Stop, Trip } from "../../domain/models";
import { tripDates } from "../../domain/dates";
import { getLegs, getPackingItems, getStops } from "../../db/trip-repository";
...
function renderBookingDetails(item: Pick<Stop | Leg, "bookingReference" | "contactInfo" | "documentUrl">) {
  const documentUrl = safeUrl(item.documentUrl);
  if (!item.bookingReference && !item.contactInfo && !documentUrl) return "";
  return `<dl class="booking">${item.bookingReference ? `<div><dt>预订号</dt><dd>${escapeHtml(item.bookingReference)}</dd></div>` : ""}${item.contactInfo ? `<div><dt>联系信息</dt><dd>${escapeHtml(item.contactInfo)}</dd></div>` : ""}${documentUrl ? `<div><dt>资料</dt><dd><a href="${escapeHtml(documentUrl)}" rel="noreferrer">打开电子票或订单</a></dd></div>` : ""}</dl>`;
}
...
function renderStop(stop: Stop) {
  ...
  return `<li class="stop">
    ...
    <div><h3>${escapeHtml(stop.title)}</h3>${location ? `<p class="location">${escapeHtml(location)}</p>` : ""}${stop.content ? `<p>${escapeHtml(stop.content)}</p>` : ""}${stop.notes ? `<p class="notes">备注：${escapeHtml(stop.notes)}</p>` : ""}${renderBookingDetails(stop)}</div>
  </li>`;
}

export function createItineraryHtml(trip: Trip, stops: Stop[], packingItems: PackingItem[], legs: Leg[] = []) {
  const days = tripDates(trip.startDate, trip.endDate).map((date) => {
    const dayStops = stops.filter((stop) => stop.date === date && !stop.unscheduled).sort((a, b) => a.sortOrder - b.sortOrder);
  ...
  const transport = legs.length ? `<section class="day"><ol>${legs.map((leg) => `<li class="stop"><time>${escapeHtml(timeLabel(leg.departsAt))}</time><div><h3>${escapeHtml(stopById.get(leg.fromStopId)?.title)} → ${escapeHtml(stopById.get(leg.toStopId)?.title)}</h3><p>${escapeHtml(leg.mode)}${leg.serviceNumber ? ` · ${escapeHtml(leg.serviceNumber)}` : ""}</p>${leg.notes ? `<p class="notes">${escapeHtml(leg.notes)}</p>` : ""}${renderBookingDetails(leg)}</div></li>`).join("")}</ol></section>` : "<p class=\"empty\">暂无交通记录</p>";
  const wantToGoStops = stops.filter((stop) => stop.unscheduled);
  const wantToGo = wantToGoStops.length ? `<section class="day"><ol>${wantToGoStops.map(renderStop).join("")}</ol></section>` : "<p class=\"empty\">暂无想去地点</p>";
