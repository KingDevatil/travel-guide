...
import { loadPackingTemplates, savePackingTemplates, type PackingTemplate } from "./templates";
...
export function PackingList({ trip, trips = [] }: { trip: Trip; trips?: Trip[] }) {
  ...
  const [templates, setTemplates] = useState<PackingTemplate[]>(loadPackingTemplates);
  const [managingTemplates, setManagingTemplates] = useState(false);
  const [pendingDelete, setPendingDelete] = useState<PackingItem>();
  ...
  const updateTemplates = (next: PackingTemplate[]) => { setTemplates(next); savePackingTemplates(next); };
  const addTemplate = async (template: PackingTemplate) => { let added = 0; for (const value of template.items) { if (!items.some((item) => item.title === value && item.category === "其他")) { await addPackingItem({ id: crypto.randomUUID(), tripId: trip.id, category: "其他", title: value, quantity: 1, required: false, packed: false, sortOrder: items.length + added }); added += 1; } } setMessage(`已从${template.name}模板添加 ${added} 项，重复项已跳过。`); await load(); };
  return <section className="feature-panel packing-list" aria-label="行李清单"><header className="feature-heading"><div><h2>行李清单</h2><p>{items.filter((item) => item.packed).length} / {items.length} 已准备</p></div><label className="check-row"><input type="checkbox" checked={hidePacked} onChange={(event) => setHidePacked(event.target.checked)} />隐藏已完成</label></header>
    ...
    <div className="packing-tools"><div className="template-tools" aria-label="快速添加模板"><div className="template-tools__heading"><span>快速模板</span><button type="button" onClick={() => setManagingTemplates(true)}>管理模板</button></div><div>{templates.map((template) => <button type="button" key={template.id} onClick={() => void addTemplate(template)}>{template.name}模板</button>)}</div></div><button type="button" disabled={!items.some((item) => item.packed)} onClick={() => void Promise.all(items.filter((item) => item.packed).map((item) => updatePackingItem({ ...item, packed: false }))).then(load)}>全部标为未准备</button>{trips.some((value) => value.id !== trip.id) && <div className="copy-packing"><label>复制其他行程清单<select value={sourceTripId} onChange={(event) => setSourceTripId(event.target.value)}><option value="">选择行程</option>{trips.filter((value) => value.id !== trip.id).map((value) => <option key={value.id} value={value.id}>{value.title}</option>)}</select></label><button type="button" disabled={!sourceTripId} onClick={() => void copyPackingItems(sourceTripId, trip.id).then((count) => { setMessage(`已复制 ${count} 项，重复项已跳过。`); return load(); })}>复制清单</button></div>}</div>{message && <p role="status">{message}</p>}
    <ul className="feature-list">{items.filter((item) => !hidePacked || !item.packed).map((item) => <li key={item.id} className={item.required && !item.packed ? "required-item" : ""}><label className="check-row"><input type="checkbox" checked={item.packed} onChange={() => void updatePackingItem({ ...item, packed: !item.packed }).then(load)} /><span><strong>{item.title} × {item.quantity}</strong><small>{item.category}{item.notes ? ` · ${item.notes}` : ""}{item.required ? " · 必需" : ""}</small></span></label><div className="row-actions"><button type="button" onClick={() => { setEditing(item); setTitle(item.title); setCategory(item.category); setQuantity(String(item.quantity)); setRequired(item.required); setNotes(item.notes ?? ""); }}>编辑</button><button type="button" className="danger-action" onClick={() => setPendingDelete(item)}>删除</button></div></li>)}</ul>
    {managingTemplates && <PackingTemplateManager templates={templates} onChange={updateTemplates} onClose={() => setManagingTemplates(false)} />}
    <ConfirmDialog open={Boolean(pendingDelete)} title="删除行李物品？" message={`将从当前行程清单中删除“${pendingDelete?.title ?? ""}”。`} confirmLabel="删除物品" onClose={() => setPendingDelete(undefined)} onConfirm={() => { if (!pendingDelete) return; void deletePackingItem(pendingDelete.id).then(load).then(() => setPendingDelete(undefined)); }} />
  </section>;
}
