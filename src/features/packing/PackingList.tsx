...
export function PackingList({ trip, trips = [] }: { trip: Trip; trips?: Trip[] }) {
  ...
  return <section className="feature-panel packing-list" aria-label="行李清单"><header className="feature-heading"><div><h2>行李清单</h2><p>{items.filter((item) => item.packed).length} / {items.length} 已准备</p></div><label className="check-row"><input type="checkbox" checked={hidePacked} onChange={(event) => setHidePacked(event.target.checked)} />隐藏已完成</label></header>
    ...
    <div className="packing-tools"><div className="template-tools" aria-label="快速添加模板"><span>快速模板</span><div>{Object.keys(packingTemplates).map((name) => <button type="button" key={name} onClick={() => void addTemplate(name as keyof typeof packingTemplates)}>{name}模板</button>)}</div></div><button type="button" disabled={!items.some((item) => item.packed)} onClick={() => void Promise.all(items.filter((item) => item.packed).map((item) => updatePackingItem({ ...item, packed: false }))).then(load)}>全部标为未准备</button>{trips.some((value) => value.id !== trip.id) && <div className="copy-packing"><label>复制其他行程清单<select value={sourceTripId} onChange={(event) => setSourceTripId(event.target.value)}><option value="">选择行程</option>{trips.filter((value) => value.id !== trip.id).map((value) => <option key={value.id} value={value.id}>{value.title}</option>)}</select></label><button type="button" disabled={!sourceTripId} onClick={() => void copyPackingItems(sourceTripId, trip.id).then((count) => { setMessage(`已复制 ${count} 项，重复项已跳过。`); return load(); })}>复制清单</button></div>}</div>{message && <p role="status">{message}</p>}
