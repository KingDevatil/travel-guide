...
export default function App() {
  ...
  return <>
    ...
    <TripWorkspace trip={currentTrip} trips={trips} activeView={activeView} onChangeView={setActiveView} onOpenManager={() => setManagerOpen(true)} managerButtonRef={managerButtonRef} />
    ...
    {managerOpen && <div className="trip-manager-overlay" role="dialog" aria-modal="true" aria-label="我的行程" onMouseDown={(event) => { if (event.target === event.currentTarget) setManagerOpen(false); }}><div ref={managerPanelRef} className="trip-manager-panel"><button className="trip-manager-close" onClick={() => { setManagerOpen(false); managerButtonRef.current?.focus(); }} aria-label="关闭行程管理">×</button>
      <TripList trips={trips} activeTripId={currentTrip.id} onSelect={(id) => { setActiveTripId(id); setManagerOpen(false); }} onCreate={() => setEditorTrip(null)} onEdit={setEditorTrip} onDuplicate={(trip) => void duplicate(trip).then(setActiveTripId)} onArchive={(trip) => void archive(trip.id).then(() => { if (trip.id === currentTrip.id) setActiveTripId(undefined); })} onRestore={(trip) => void restore(trip.id).then(() => setActiveTripId(trip.id))} onDelete={setPendingDelete} />
    ...
    <ConfirmDialog open={Boolean(pendingDelete)} title="删除行程？" message={`将永久删除“${pendingDelete?.title ?? ""}”及其所有节点、交通、消费和行李数据。`} confirmLabel="删除" onClose={() => setPendingDelete(undefined)} onConfirm={() => { if (pendingDelete) void remove(pendingDelete.id).then(() => { if (pendingDelete.id === currentTrip.id) setActiveTripId(undefined); setPendingDelete(undefined); }); }} />
  </>;
}
