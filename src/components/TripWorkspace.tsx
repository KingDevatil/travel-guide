...
import { Backpack, BookOpen, CalendarDays, ChevronDown, ChevronRight, Download, ListFilter, Map, MapPin, MoreHorizontal, MoreVertical, Plane, Receipt, Search, Settings, ShieldCheck, Train, UtensilsCrossed, Users } from "lucide-react";
...
export function TripWorkspace({ trip, trips, activeView, onChangeView, onOpenManager, onOpenParticipants, managerButtonRef }: { trip: Trip; trips: Trip[]; activeView: ViewMode; onChangeView: (view: ViewMode) => void; onOpenManager: () => void; onOpenParticipants: () => void; managerButtonRef: React.RefObject<HTMLButtonElement | null> }) {
  ...
  return <div className="planner-shell">
    <header className="planner-topbar"><div className="planner-brand"><BookOpen /><span>旅程册</span></div><div className="planner-divider" /><button className="planner-trip" onClick={onOpenManager}><strong>{trip.title}</strong><ChevronDown /><small>{dateLabel(trip.startDate)}—{dateLabel(trip.endDate)}</small></button><nav aria-label="主要功能">{views.map(({ value, label, icon: Icon }) => <button key={value} className={activeView === value ? "active" : ""} aria-current={activeView === value ? "page" : undefined} onClick={() => onChangeView(value)}><Icon /><span>{label}</span></button>)}</nav><div className="planner-tools"><button aria-label="管理成员" onClick={onOpenParticipants}><Users /></button><button ref={managerButtonRef} onClick={onOpenManager} aria-label="管理行程"><Settings /></button><button aria-label="管理安排" onClick={() => setPlanningOpen(true)}><MoreHorizontal /></button></div></header>
    <main className="planner-main">
      {activeView === "itinerary" ? <><aside className="planner-days"><button className="planner-add" onClick={() => setAdding(true)}>添加安排 <span>＋</span></button><div className="planner-day-list">{days.map((date, index) => { const isActive = date === selectedDay; const items = stops.filter((stop) => stop.date === date); return <button key={date} className={isActive ? "selected" : ""} aria-pressed={isActive} onClick={() => { setSelectedDay(date); setSelectedStop(undefined); }}><i className="day-track">{index < days.length - 1 && <em />}</i><span><strong>{dateLabel(date)}</strong><b>{weekday(date)}</b><small aria-hidden="true">{items[0]?.city || "待安排"}</small><small>{items.length} 项安排</small></span><ChevronRight /></button>; })}</div><div className="planner-rail-tools"><button aria-label="回到旅行第一天" onClick={() => { setSelectedDay(trip.startDate); setSelectedStop(undefined); }}><CalendarDays /></button><button aria-label="管理安排" onClick={() => setPlanningOpen(true)}><ListFilter /></button><button aria-label="搜索安排" onClick={() => { setSearchText(""); setSearchOpen(true); }}><Search /></button></div></aside>
      ...
      <section className="planner-map"><header><div><button className={mapRange === "full" ? "active" : ""} aria-pressed={mapRange === "full"} onClick={() => setMapRange("full")}>全览</button><button className={mapRange === "day" ? "active" : ""} aria-pressed={mapRange === "day"} onClick={() => setMapRange("day")}>当天</button><button onClick={() => setMapFitRequest((value) => value + 1)}>适配范围</button></div><button aria-label="打开全屏地图" onClick={() => onChangeView("map")}>⛶</button></header><MapArtwork stops={stops} legs={legs} day={mapRange === "day" ? selectedDay : undefined} selectedStopId={selectedStop} fitRequest={mapFitRequest} onSelectStop={(id) => { setSelectedStop(id); const stop = stops.find((item) => item.id === id); if (stop) setSelectedDay(stop.date); }} /><footer><span><MapPin /> 已选择：{selected?.title || "暂无节点"}</span><button disabled={!selected} onClick={() => selected && setEditingStop(selected)}>查看详情 <ChevronRight /></button></footer></section></> : activeView === "map" ? <TripMapView trip={trip} /> : activeView === "expenses" ? <ExpenseList trip={trip} /> : <PackingList trip={trip} trips={trips} />}
