...
当前验收基线包含 10 个测试文件、138 项自动化测试，覆盖领域模型、城市匹配、数据持久化、导入导出、地图 GeoJSON、费用分摊和关键业务工作流。

## 数据与隐私

应用数据保存在当前浏览器的 IndexedDB 中，不会自动上传。清除站点数据、切换浏览器或更换设备前，请先在“备份”页面导出 JSON 文件。导入操作会先进行结构校验，并允许选择覆盖或保留冲突数据。

地图底图需要网络连接；行程管理、消费、行李清单和已保存数据的读取可在本地完成。本项目不提供云同步、实时汇率、导航或预订服务。
